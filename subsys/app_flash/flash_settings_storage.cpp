...
void fss_write_data(uint8_t *data,uint16_t size){
	if (flash_erase(flash_dev, SETTINGS_PARTITION_OFFSET, FLASH_PAGE_SIZE) != 0) {
		printf("   Flash erase failed!\n");
	} else {
		printf(" * Succeeded Flash erase partition 'settings_storage' @0x%x 4KB Page!\n",SETTINGS_PARTITION_OFFSET);
	}
    off_t offset = SETTINGS_PARTITION_OFFSET;
...
void fss_read_data(uint8_t *data,uint16_t size){
    ...
    const off_t offset = SETTINGS_PARTITION_OFFSET;
