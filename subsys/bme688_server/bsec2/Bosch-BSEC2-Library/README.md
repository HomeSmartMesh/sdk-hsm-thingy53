...
dropped content
- examples directory dropped
- all targets dropped except cortex-m33 left for the Thingy_53 target hardware
- `extra_script.py` that reads env `BOARD_MCU` and sets `LIBPATH` and `LIBS` to `agobsec`
- `src\inc\bsec_interface_multi.h` because the Thingy_53 has only one BME688 sensor
- `bsec2.h` and `bsec2.cpp` are cpp Arduino wrappers replaced by the `bme688_server.h` and `bme688_server.cpp` cpp Zephyr wrapper which wraps both of `BME68x-Sensor-API` and `Bosch-BSEC2-Library`

for usage with any other target than Thingy_53 please refer to the above referenced repo

the below section is unmodified
---
